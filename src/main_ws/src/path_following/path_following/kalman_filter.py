...
class KalmanFilter(Node):
    ...
    def state_estimate(self, xs):
        ...
        x = xs.position.x
        y = xs.position.y
        psi = xs.position.theta
        u = xs.velocity.u
        v = xs.velocity.v
        r = xs.velocity.r

        EKF = ExtendedKalmanFilter(6,3,2,x,y,psi,u,v,r)
...
class ExtendedKalmanFilter(object):
    ...
    def __init__(self, dim_x, dim_z, dim_u, x_pos, y_pos, psi, u, v, r):

        self.dim_x = dim_x
        self.dim_z = dim_z
        self.dim_u = dim_u

        self.x = np.array([[x_pos],[y_pos],[psi],[u],[v],[r]]) # state
